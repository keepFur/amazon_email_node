...
KbOrderManage.prototype.createKbOrder = function (data) {
    let cmdText = `INSERT INTO yidian_kb_order
        (number,remark, plant,user_id,kb_number,kb_company, address_from, address_to, address_to_pca,to_name,to_phone, total,created_date) VALUES `,
        cmdParams = [];
    if (Array.isArray(data.kbNumber)) {
        data.kbNumber.forEach(function (number, index) {
            ...
            cmdParams.push(data.plant === 'PDD' ? data.addressTo[index].split(/，|,/)[3] : data.number);
...
KbOrderManage.prototype.readKbOrderPage = function (data) {
    ...
    let cmdText = `SELECT 0.id,o.number,remark,plant, address_from AS addressFrom,address_to AS addressTo, address_to_pca AS addressToPca,to_name AS toName ,to_phone AS toPhone , total,kb_number AS kbNumber,kb_company AS kbCompany, o.created_date AS createdDate,o.update_date AS updateDate 
...
KbOrderManage.prototype.readKbOrderById = function (data) {
    let cmdText = `SELECT 0.id,o.number,remark,plant, address_from AS addressFrom,address_to AS addressTo, to_name AS toName ,to_phone AS toPhone, address_to_pca AS addressToPca, total,kb_number AS kbNumber,kb_company AS kbCompany,o.created_date AS createdDate,o.update_date AS updateDate 
