...
KbOrderManage.prototype.createKbOrder = function (req, res, data) {
    //  判断用户余额是否足够
    // 计算会员的价格
    data.total = Core.flyer.computeTotalPrice(req.user.level, data.total);
    ...
    DB.dbQuery.manageKbOrder.toggleKbOrder(data).then(result => {
        if (data.status === 3) {
            ...
            DB.dbQuery.userManage.addMoneyUser({
                ...
                // 增加退款日志
                DB.dbQuery.userManage.readUserById({ id: data.userId }).then(user => {
                    if (user.length) {
                        DB.dbQuery.logsScoreManage.createLogsScore({
                            userId: data.userId,
                            ...
                            count: data.count,
                            orderNumber: data.number,
                            balance: parseInt(user[0].money) + parseInt(data.count)
                        }).then(function (logResult) {
                            res.send({
...
function downloadErrFile(res, content) {
    let errFilePath = path.join(path.resolve(), mkdir_templeate + 'error.docx');
...
function turnToUtf8(filePath) {
    var { encoding, buffer } = getFileEncoding(filePath);
    var byte = buffer;
    // 为gbk文件
    if (encoding === 'gbk') {
        var c = iconv.decode(byte, 'gbk');
...
KbOrderManage.prototype.importAddressExcel = function (req, res, file) {
    if (!file.length) {
        ...
        try {
            ...
            // if (encoding === 'gbk') {
            //     detail = JSONDatas[0][getKeyByPlant(plant).detail];
            //     detail = iconv.decode(new Buffer(detail), 'gbk');
            // }

            // 为gbk编码
            // if (encoding === 'gbk') {
            //     JSONDatas.map(function (item) {
            //         for (const key in item) {
            //             if (item.hasOwnProperty(key)) {
            //                 const element = item[key];
            //                 item[iconv.decode(new Buffer(key), 'gbk')] = iconv.decode(new Buffer(element), 'gbk');
            //             }
            //         }
            //         // item = iconv.decode(new Buffer(item.toString()), 'gbk');
            //         return item;
            //     });
            // }
            // 为gbk编码
            // if (encoding === 'gbk') {
            //     var item = JSONDatas[0];
            //     for (const key in item) {
            //         if (item.hasOwnProperty(key)) {
            //             const element = item[key];
            //             const decoder = new StringDecoder('utf8');
            //             // item[iconv.decode(new Buffer(key), 'gbk')] = iconv.decode(new Buffer(element), 'gbk');
            //             item[decoder.write(new Buffer(key))] = decoder.write(new Buffer(element));
            //         }
            //     }
            // }
            var detail = JSONDatas[0][getKeyByPlant(plant).detail];
            if (!detail) {
                res.send({
                    success: false,
                ...
                let datas = JSONDatas.map((item) => {
                    ...
                    //             const decoder = new StringDecoder('utf8');
                    //             // item[iconv.decode(new Buffer(key), 'gbk')] = iconv.decode(new Buffer(element), 'gbk');
                    //             item[decoder.write(new Buffer(key), 'utf-8')] = decoder.write(new Buffer(element), 'utf-8');
                    //         }
                    //     }
                    // }
...
KbOrderManage.prototype.importKbNumberExcel = function (req, res, file) {
    if (!file.length) {
        ...
        try {
            ...
            // 再去数据库判重
            uniqueKbNumber(datas, { numbers: datas.map(i => i.number) }, function (ret) {
                if (ret.length === 0) {
                    res.send({
                        success: true,
                        message: '空包单号创建成功'
