...
// 创建空包订单(拼多多电子面单专用)
KbOrderManage.prototype.createPDDDZKbOrder = function (req, res, data) {
    // 计算会员的价格
    data.price = Core.flyer.computeTotalPrice(req.user.level, data.price);
    data.total = data.price * data.addressTo.length;
    //  判断用户余额是否足够
    DB.dbQuery.userManage.readUserById({
        ...
        if (user && user.length > 0 && user[0].money >= data.total) {
            data.userId = req.user.id;
            DB.dbQuery.manageKbOrder.createPDDDZKbOrder(data).then(result => {
...
KbOrderManage.prototype.downloadKbOrderToExcel = function (req, res, data) {
    ...
    DB.dbQuery.manageKbOrder.readKbOrderPage(data).then(result => {
        ...
        let fileName = getKbTypeByCode(data.kbCompany) + '-' + getPlantByCode(data.plant) + '-' + Core.flyer.formatDate('yyyy-mm-dd') + '-待扫描(共' + temp.length + '单).xls',
            filePath = path.join(path.resolve(), mkdir_order + fileName);
        temp.unshift(columns);
        let workbook = make_book(fileName, temp);
        XLSX.writeFileAsync(filePath, workbook, () => {
...
/**
 *获取一个地址中的省市区(拼多多电子面单专用)
 *
 * @param {*} add
 */
function getPDDDZPCAByAddress(item, printType) {
    let key = getKeyByPrintType(printType);
    let detail = item[key.detail];
    let p = '';
    let c = '';
    let a = '';
    ...
    if (printType === 'CHM') {
        ...
        a = item[key.area];
        d = item[key.detail];
    } else if (printType === 'CJDZ') {
        p = detail.split(' ')[0];
        c = detail.split(' ')[1];
        a = detail.split(' ')[2];
        d = detail.split(' ')[3];
    } else if (printType === 'DD') {
        const allP = APIArea.getProvinceServer().rows;
        allP.forEach(element => {
            if (detail.indexOf(element.name.replace('省', '')) !== -1) {
                p = element.name.replace('省', '');
...
// 获取拼多多电子面单打印类型
function getPDDDZPrintType(content){
    let prints = {
        '订单号':'CHM',
        '序号' :'CJDZ',
        '发货时间':'DD'
    };
    return prints[Object.keys(content)[0]];
}

// 根据打印类型获取表格的key
function getKeyByPrintType(printType) {
    printType = printType || 'TB';
    var key = {
        CHM: {
            name: '收货人姓名',
            phone: '收件人手机',
    ...
    return key[printType];
}

// 通过excel导入发货地址(拼多多电子面单专用)
KbOrderManage.prototype.importPDDDZAddressExcel = function (req, res, file) {
    if (!file.length) {
        res.send({
            success: false,
            message: '没有选择文件'
        ...
        try {
            APIParseExcel(filePath, function (err, resdata) {
                if (err) {
                    ...
                    // 1，判断出是用的哪一种打印，就跟判断平台是一样的
                    console.log(JSONDatas[0]);
                    let printType =JSONDatas.length&& getPDDDZPrintType(JSONDatas[0]);
                    // 2，找出对应的打印软件之后，再获取详细的信息
                    if (!printType) {
                        res.send({
                            success: false,
                            message: '请选择正确的表格进行导入。',
                        });
                    } else {
                        let datas = JSONDatas.map((item) => {
                            var address = getPDDDZPCAByAddress(item, printType);
                            const key = address.key;
                            return {
                                name: item[key.name],
                                phone: item[key.phone],
