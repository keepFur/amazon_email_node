...
LogsScoreManage.prototype.createLogsScore = function (data) {
    let cmdText = `INSERT INTO yidian_logs_score
        (user_id,user_name,type,count,balance, order_number,created_date)
        VALUES (?,?,?,?,?,?,?)`,
        cmdParams = [];
...
LogsScoreManage.prototype.readLogsScorePage = function (data) {
    let limit = Number(data.limit || 20),
        ...
        cmdText = `SELECT score.id, user_id AS userId,user_name AS userName, type,count,balance, order_number AS orderNumber, score.created_date AS createdDate
        ,score.status FROM yidian_logs_score score LEFT JOIN yidian_user user ON score.user_id=user.id WHERE 1=1 `,
        cmdParams = [];
    // 普通用户只能看自己的
    if (!data.isSuper) {
        cmdText += ` AND user_id = ?`;
...
LogsScoreManage.prototype.readLogsScoreById = function (data) {
    let cmdText = `SELECT id, user_id AS userId,user_name AS userName, type,count,balance,order_number AS orderNumber , created_date AS createdDate
