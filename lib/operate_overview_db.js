...
OperateOverview.prototype.getTodayData = function () {
    ...
    SELECT COUNT(*) AS todayAddKbOrderCount ,SUM(total) AS todayAddKbOrderSumMoney FROM yidian_kb_order WHERE to_days(created_date) = to_days(now()); `,
        cmdParams = [];
...
OperateOverview.prototype.getAllData = function () {
    ...
    SELECT COUNT(*) AS todayAddKbOrderCount,SUM(total) AS allAddKbOrderSumMoney FROM yidian_kb_order;`,
        cmdParams = [];
    return Core.flyer.return_promise(this.pool, cmdText, cmdParams);
};
...
OperateOverview.prototype.getUserData = function (data) {
    ...
    SELECT COUNT(*) AS purTaskUserCount FROM yidian_task t INNER JOIN yidian_user u ON u.id = t.task_user_id WHERE DATE_SUB(CURDATE(), INTERVAL ? DAY) <= date(u.created_date);
    SELECT COUNT(*) AS purKbUserCount FROM yidian_kb_order o INNER JOIN yidian_user u ON u.id = o.user_id WHERE DATE_SUB(CURDATE(), INTERVAL ? DAY) <= date(u.created_date);`,
