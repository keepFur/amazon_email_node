...
TbTaskDB.prototype.createTask = function (data) {
    ...
    cmdParams.push(Core.flyer.formatDate('yyyy-mm-dd hh:MM:ss'));
...
TbTaskDB.prototype.readTaskPage = function (data) {
    let limit = Number(data.limit || 20),
        ...
        cmdText = `SELECT task.id,task_user_id AS taskUserId,u.username AS taskUserName, task_name AS taskName,task_parent_type AS taskParentType,
        task_child_type AS taskChildType,type.name AS taskTypeName,task_keyword AS taskKeyword,task_quantity AS taskQuantity,
        task_start_date AS taskStartDate, task.status,task_order_number AS taskOrderNumber,task_plant AS taskPlant,
        task_baby_link_token AS taskBabyLinkToken, task_unit_price AS taskUnitPrice,task_sum_money  AS taskSumMoney,
        task.created_date AS createdDate,task.update_date AS updateDate 
        FROM yidian_task task INNER JOIN yidian_task_type type ON type.id=task.task_child_type INNER JOIN yidian_user u ON u.id = task.task_user_id WHERE 1 = 1`,
        cmdParams = [];
    if (data.id) {
        cmdText += ` AND task.id = ?`;
    ...
    if (data.taskOrderNumber) {
        cmdText += ` AND task_order_number LIKE '%${data.taskOrderNumber}%'`;
    }
    // 普通用户只能看自己的任务
    if (!data.isSuper) {
    ...
    cmdText += ` ORDER BY task.created_date DESC LIMIT ?,?`;
...
// 读取全部状态为处理中任务
TbTaskDB.prototype.readAllProcessTask = function (data) {
    let cmdText = `SELECT id, task_order_number AS taskOrderNumber FROM yidian_task WHERE status = 1`,
        cmdParams = [];
    // 普通用户只能看自己的任务
    if (!data.isSuper) {
        cmdText += ` AND task_user_id = ?`;
...
TbTaskDB.prototype.readTaskById = function (data) {
    let cmdText = `SELECT id,task_user_id,task_user_id AS taskUserId, task_name AS taskName,task_parent_type AS taskParentType,
...
// 根据任务订单号，奖任务标记为已完成
TbTaskDB.prototype.maskCompleteTask = function (data) {
    let cmdText = `UPDATE  yidian_task SET status = ? WHERE task_order_number IN ( ${data.id} )`,
        cmdParams = [data.status];
    return Core.flyer.return_promise(this.pool, cmdText, cmdParams);
};
