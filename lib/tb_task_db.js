...
TbTaskDB.prototype.readTaskPage = function (data) {
    let limit = Number(data.limit || 20),
        ...
        cmdText = `SELECT task.id,task_user_id AS taskUserId, task_name AS taskName,task_parent_type AS taskParentType,
        ...
        FROM yidian_task task INNER JOIN yidian_task_type type ON type.id=task.task_child_type WHERE 1 = 1`,
        cmdParams = [];
    if (data.id) {
        cmdText += ` AND task.id = ?`;
