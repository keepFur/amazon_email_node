...
ManageKbAddress.prototype.readKbAddress = function (data) {
    let cmdText = `SELECT id, detail,email, contact,phone,remark, p_code AS pCode,c_code AS cCode ,a_code AS aCode, pca,is_default AS isDefault, created_date AS createdDate,update_date AS updateDate 
...
// 设置默认地址
ManageKbAddress.prototype.setDefaultKbAddress = function (data) {
    let cmdText = `UPDATE  yidian_kb_address SET is_default = 1 WHERE id = ? ; UPDATE  yidian_kb_address SET is_default = 0 WHERE user_id = ? AND id != ?`,
        cmdParams = [data.id, data.userId, data.id];
    return Core.flyer.return_promise(this.pool, cmdText, cmdParams);
};
