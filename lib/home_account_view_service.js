...
// 读取某一段时间的空包购买数量
HomeAccountView.prototype.readKbCountOfInTime = function (req, res, data) {
    data.userId = req.user.id;
    data.isSuper = req.user.isSuper;
    DB.dbQuery.homeAccountView.readKbCountOfInTime(data).then(result => {
        res.send({
            success: true,
            data: {
                rows: result
...
//  读取某一段时间的购买空包类型
HomeAccountView.prototype.readKbTypeOfInTime = function (req, res, data) {
    data.userId = req.user.id;
    data.isSuper = req.user.isSuper;
    DB.dbQuery.homeAccountView.readKbTypeOfInTime(data).then(result => {
        res.send({
            success: true,
            message: '',
            data: {
