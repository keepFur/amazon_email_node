...
HomeAccountView.prototype.readTaskTypeOfInTime = function (data) {
    let cmdText = `SELECT  count(id) as count ,task_child_type AS taskChildType from yidian_task task WHERE 1=1 `,
        cmdParams = [];
    // 普通用户只能看读自己创建的任务
    if (!data.isSuper) {
        cmdText += ` AND task_user_id = ?`;
...
HomeAccountView.prototype.readKbTypeOfInTime = function (data) {
    let cmdText = `SELECT  count(id) as count,plant,kb_company AS kbCompany from yidian_kb_order o  WHERE 1=1 `,
        cmdParams = [];
    // 普通用户只能看读自己创建的任务
    if (!data.isSuper) {
        cmdText += ` AND user_id = ?`;
    ...
    cmdText += ` GROUP BY o.plant,o.kb_company`;
    return Core.flyer.return_promise(this.pool, cmdText, cmdParams);
};
