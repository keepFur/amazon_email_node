...
ManageKbNumber.prototype.readKbNumberByNumber = function (data) {
    let cmdText = `SELECT * FROM  yidian_kb_number  WHERE number IN ${Core.flyer.convertParams(data.numbers)} `,
...
ManageKbNumber.prototype.readKbNumberStock = function (data) {
    let cmdText = `select company,sum(if( status=1,1,0)) as remain,sum(if( status=0,1,0)) as used ,count(id)  as total from yidian_kb_number where  plant= ? group by company`,
        cmdParams = [data.plant];
    return Core.flyer.return_promise(this.pool, cmdText, cmdParams);
}
