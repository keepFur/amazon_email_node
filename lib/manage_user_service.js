...
UserManage.prototype.createUser = function (req, res, data) {
    ...
    if (req.session.code != data.verfiyCode) {
        res.send({
            success: false,
            message: '验证码错误'
        });
...
// 获取用户推广详情信息
UserManage.prototype.readUserExtendDetailById = function (req, res, data) {
    DB.dbQuery.userManage.readUserExtendDetailById(data).then(result => {
        res.send({
            success: true,
            message: '',
            data: {
                registerOrigin: result[0].length ? '（' + result[0][0].username + '）推荐' : '一级代理',
                extendMoney: result[1][0].allExtendSumMoney
            }
        });
    }).catch(err => res.send({
        success: false,
