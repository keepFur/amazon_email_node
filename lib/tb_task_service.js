...
TbTaskService.prototype.createTask = function (req, res, data) {
    //  判断用户余额是否充值
    // 计算会员的价格
    data.taskSumMoney = Core.flyer.computeTotalPrice(req.user.level, data.taskSumMoney);
    data.taskUnitPrice = Core.flyer.computeTotalPrice(req.user.level, data.taskUnitPrice);
                ...
                if (result.affectedRows === 1) {
                    ...
                    DB.dbQuery.userManage.reduceMoneyUser({
                        ...
                        DB.dbQuery.logsScoreManage.createLogsScore({
                            ...
                            balance: parseInt(req.user.money) - parseInt(data.taskSumMoney)
        ...
        if (data.status == 3) {
            ...
            DB.dbQuery.userManage.addMoneyUser({
                ...
                DB.dbQuery.logsScoreManage.createLogsScore({
                    ...
                    count: data.count,
                    orderNumber: data.taskOrderNumber,
                    balance: parseInt(req.user.money) + parseInt(data.count)
                }).then(function (logResult) {
                    res.send({
                        success: logResult.affectedRows === 1,
                        message: ''
                    });
                    console.log(`在${new Date()},用户： ${req.user.userName}充值${data.count}`);
                });
            }).catch(err => res.send({ success: false, message: err.message }));
        } else {
            res.send({
