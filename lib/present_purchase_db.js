...
PresentPurchase.prototype.createPresentOrder = function (data) {
    let cmdText = `INSERT INTO yidian_present_order
        ...
        cmdParams = [];
    if (Array.isArray(data.addressToLocal)) {
        data.addressToLocal.forEach(function (number, index) {
            if (index !== data.addressToLocal.length - 1) {
                cmdText += '(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?),';
            } else {
                cmdText += '(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
            }
            ...
            cmdParams.push(data.addressToLocal[index]);
            cmdParams.push(data.addressToPcaLocal[index]);
            cmdParams.push(data.addressFromPhone);
            cmdParams.push(data.addressToLocal[index].split(/，|,/)[0]);
            cmdParams.push(data.addressToLocal[index].split(/，|,/)[1]);
