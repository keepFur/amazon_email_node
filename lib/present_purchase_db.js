...
PresentPurchase.prototype.readPresentOrderNoKdNumber = function () {
    let cmdText = `SELECT id,order_number AS orderNumber,user_id AS userId,taskid,created_date AS createdDate FROM yidian_present_order WHERE kd_number = '000000' ORDER BY created_date ASC`,
        cmdParams = [];
    return Core.flyer.return_promise(this.pool, cmdText, cmdParams);
};
...
PresentPurchase.prototype.updatePresentOrderById = function (data) {
    let cmdText = `UPDATE  yidian_present_order SET kd_number = ? ,update_date = ? WHERE id = ? AND taskid = ?`,
