...
layui.use(['form', 'element', 'table', 'layer', 'util', 'upload'], function() {
            ...
            function initEvent() {
                ...
                $('textarea[name=addressTo]').on('blur', function(e) {
                    ...
                    var sumMoney = quantity && baseDatas.presentInfo ? quantity * baseDatas.presentInfo.price : 0;
                    $('#kbQuantity').text(quantity);
                    // 总价
                    $('#kbSumMoney').text(core.fenToYuan(sumMoney));
                    // 优惠总价
                    $('#discountKbSumMoney').text(core.fenToYuan(core.computeTotalPrice(baseDatas.level, baseDatas.presentInfo ? baseDatas.presentInfo.price : 0) * quantity));
                    return false;
                });
                // 通过excel导入收货地址
            ...
            for(let i = 0;i< presentOrderInfo.addressTo.length;i++){
                (function(i){
                    ...
                    APIUtil.createPresentOrder({
                            ...
                            receiver_address:presentOrderInfo. addressToPca[i].split('-')[3],
                        ...
                        presentOrderInfo.taskid = res.data.data.taskid;
    ...
    function renderPrensentList(prensentList) {
        ...
        for (var index = 0; index < prensentList.length; index++) {
            ...
            tpl += `<div class="layui-col-xs3">
                                <div class="present-item" data-id="${present.id}" data-name="${present.name}" data-price="${present.price}">
                                    <div class="present-item-body">
                                        <img src="./imgs/test.jpg" width="100%" height="100%" alt="礼品">
                                    </div>
                                    <div class="present-item-footer">
                                        <h5 style="margin-bottom:8px;">${present.name}</h5>
                                        <div class="layui-row layui-col-space8">
                                            <div class="layui-col-xs4">
                                                <span style="color:red"> ¥${core.fenToYuan(present.price)}</span>
                                            </div>
                                            <div class="layui-col-xs4">
        ...
        $('.present-item').click(function(e) {
            ...
            baseDatas.presentInfo = { id, name, price };
    ...
    function renderPresentSelect(presents) {
        ...
        $.each(presents, function(index, item) {
            ...
            $container.append(`<option value="${item.id}" data-price="${item.price}" data-name="${item.name}" ${baseDatas.presentInfo&&item.id==baseDatas.presentInfo.id?'selected':''}>${item.name} ，你的价格（${levelText}）：${curP}元，普通会员：${comP}元，金牌会员：${goldP}元</option>`);
        });
        form.render('select');
    }
    ...
    function readPresentListServer() {
        $.ajax({
            ...
            success:function (data) {
                baseDatas.prensentList = data.data.rows;
                renderPresentSelect(baseDatas.prensentList);
    ...
    function getPresentFromStockServer() {
        $.get('/api/readFromStock?status=1', function (res) {
            renderFromStockSelect(res.data.rows);
        }, 'json');
    }
    ...
    function renderFromStockSelect(stocks) {
        ...
        $.each(stocks, function(index, item) {
            $container.append(`<option value="${item.id}"} ${index==0?'selected':''}>${item.name}   发货地址：${item.address}</option>`);
        });
        form.render('select');
    }
