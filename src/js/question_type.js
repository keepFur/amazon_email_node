...
flyer.define('countries', function (exports, module) {
    ...
    function initEvent() {
        // 创建问题分类（支持批量）
        $('.createQusType').on('click', createQusTypeHandle);
        // 删除问题分类（支持批量）
        $('.deleteQusType').on('click', deleteQusTypeHandle);
    }

    /**
     * 创建商品品类（支持批量）的点击事件处理函数
     * 
     * @param {any} events 
     */
    function createQusTypeHandle(events) {
        flyer.open({
            pageUrl: core.url + '/html/add_base_data.html',
            isModal: true,
            area: [440, 160],
            title: flyer.i18n.initTitle('创建问题分类'),
            btns: [{
                text: flyer.i18n.initTitle('保存'),
                click: function () {
                    var that = this, $firstBtn = $(that.$btns[0]),
                        questionType = flyer.exports.baseData.unniqueArr(flyer.exports.baseData.toArray($('#baseDataName').val().trim()));
                    if (questionType) {
                        questionType = hasQuestionType(questionType, JSON.parse(flyer.store.get('questionType')).map(function (item) {
                            return item.questionType;
                        }));
                        if (questionType.length) {
                            $.ajax({
                                url: core.url + '/create_question_type',
                                type: 'POST',
                                data: {
    ...
    /**
     * 删除问题分类按钮点击事件处理函数(批量删除)
     * 
     * @param {any} events 
     */
    function deleteQusTypeHandle(events) {
            ...
            flyer.confirm(flyer.i18n.initTitle('确定删除吗?'), function (result) {
                    ...
                    btns: [{
                        ...
                        click: function (elm) {
                            ...
                            $.ajax({
                                url: core.url + '/delete_question_type',
                                type: 'POST',
                                data: {
                                    IDS: IDS
                                },
     ...
     * @param {any} events 
     */
    function deleteQusTypeSingleHandle(events) {
     ...
     * @param {any} events 
     */
    function updateQusTypeHandle(events) {
        var questionType = $(events.target).data('questiontype'),
            open = flyer.open({
                ...
                + '<input type="text" id="baseDataName" required="true" autocomplete="off" class="flyer-input" placeholder=' + flyer.i18n.initTitle('原名称') + '：' + questionType + ' value="' + questionType + '" autofocus>'
                + '</div>'
                + '</div>'
                + '</div>',
                isModal: true,
                area: [440, 160],
                title: flyer.i18n.initTitle('更新问题分类'),
                btns: [{
                    text: flyer.i18n.initTitle('保存'),
                    click: function () {
                        var that = this, $firstBtn = $(that.$btns[0]),
                            ID = Number($(events.target).data('id')),
                            questionType = $('#baseDataName').val().trim();
                        if (questionType) {
                            questionType = hasQuestionType([questionType], JSON.parse(flyer.store.get('questionType')).map(function (item) {
                                return item.questionType;
                            }));
                            if (questionType.length) {
                                $.ajax({
                                    url: core.url + '/update_question_type',
                                    type: 'POST',
                                    data: {
                                        ID: ID,
        ...
        if ($table && $table.length && Array.isArray(datas)) {
            baseDatas.$table = flyer.table($table, {
                columns: [{
                    ...
                    formatter: function (row) {
                        return '<span class="table-btn" title="' + flyer.i18n.initTitle("编辑") + '"><i class="icon-pen table-btn flyer-editquestion-btn" data-id="' + row.ID + '" data-questiontype="' + row.questionType + '" style="padding-right:8px;"></i></span>'
                            + '<span class="table-btn" style="display:none;" title="' + flyer.i18n.initTitle("删除") + '"><i class="icon-remove table-btn flyer-deletequestion-btn" data-id="' + row.ID + '" style="padding-right:8px;"></i></span>'
                    }
                }],
                data: datas
            });
    ...
    function randerDOMPager($table, datas, total, pagerObj) {
        // 没有数据的时候
        core.tableNoMatch($table, flyer.i18n.initTitle('暂时没有相关问题分类记录'));
        // 初始化下拉框，显示每页数据条数的下拉框
        baseDatas.pageSizeSelectObj = core.initPagerSizeSelect($('#qusTypePagerSize'), core.getPageListByTotal(total), String(pagerObj.pageSize || 20), {
        ...
        // 初始化分页
        baseDatas.pagerObj = core.initPager($('.paper-container-question'), total, pagerObj.pageSize || 20, {
            callback: getTableDatas,
    ...
    function setMountValue(currentTotal, total) {
        $('#currentQusTypeMountSpan').text(currentTotal);
        $('#qusTypeMountSpan').text(total);
    }
    ...
    function getTableDatas(pageNumber, pageSize) {
        ...
        $.ajax({
            url: core.url + '/read_question_type_page',
            type: 'GET',
            data: conditions,
            beforeSend: function (jqXHR, settings) {
            ...
            success: function (data, jqXHR, textStatus) {
                if (data.success) {
                    ...
                    getQuestionType();
                    // 修改问题分类
                    $('.flyer-editquestion-btn').off('click').on('click', updateQusTypeHandle);
                    // 删除问题分类(单个操作)
                    $('.flyer-deletequestion-btn').off('click').on('click', deleteQusTypeSingleHandle);
                } else {
                    flyer.msg(baseDatas.netErrMsg);
                    renderTable($table, []);
                }
    ...
    /**
        * 获取所有的问题类型数据，并缓存到浏览器中
        * 
        */
    function getQuestionType() {
        $.ajax({
            url: core.url + '/read_question_type',
            type: 'GET',
            data: {
                nocache: window.Date.now(),
                companyOrgID: baseDatas.companyOrgID
            ...
            success: function (data, jqXHR, textStatus) {
                flyer.store.set('questionType', JSON.stringify(data.success ? data.data.rows : []));
            },
            error: function (jqXHR, textStatus, errorThrown) {
                flyer.msg(baseDatas.netErrMsg);
            }
