...
flyer.define("advanced_search", function (exports, module) {
    ...
    AdvancedSearch.prototype.initPage = function () {
        if (window.location.hash === '#unassigned' || window.location.hash === '#sent_emails' || core.getUserGroups().orgCode === '9101') {
            $('.public-butModal-people').parents('.public-butModal-inputBox').hide();
        }
    };

    // 初始化下拉框
    AdvancedSearch.prototype.ininSelectBox = function () {
        var that = this, startDate = null, endDate = null
            ;
        this.clearConditions();
        // 处理人
        if (core.getUserGroups().orgCode !== '9101') {
            if (window.location.hash !== '#unassigned') {
                this.HandlePeople = flyer.combobox($('.public-butModal-people'), {
                    isMulti: true,
                    selectAll: false,
        ...
        // 邮箱名
        this.emailName = flyer.combobox($('.public-butModal-email'), {
            isMulti: true,
        ...
        // 开始时间
        startDate = flyer.date($('#public-butModal-startTime'), {
            //最大允许选择的时间范围
            isTime: false,
            format: 'yyyy-mm-dd',
        ...
        // 结束时间
        endDate = flyer.date($('#public-butModal-endTime'), {
            //最大允许选择的时间范围
            isTime: false,
            format: 'yyyy-mm-dd',
            choose: function (dateStr, dateObj) {
    ...
    AdvancedSearch.prototype.getConditions = function () {
        ...
        // 开始时间
        if ($('.public-butModal-startTime').length) {
            resultObj.startDate = $('.public-butModal-startTime').val().trim();
        }
        if (resultObj.startDate) {
            resultObjText += '，' + flyer.i18n.initTitle('开始时间') + '[' + resultObj.startDate + ']';
        }
        // 结束时间
        if ($('.public-butModal-endTime').length) {
            resultObj.endDate = $('.public-butModal-endTime').val().trim();
        }
        if (resultObj.endDate) {
            resultObjText += '，' + flyer.i18n.initTitle('结束时间') + '[' + resultObj.endDate + ']';
        }
        // 关键字
        if ($('.public-butModal-search input[name=keyword]').length) {
            resultObj.keyword = $('.public-butModal-search input[name=keyword]').val().trim().replace(/\?/, '？');
        }
    ...
    AdvancedSearch.prototype.clearConditions = function () {
        ...
        $('.public-butModal-endTime').val('');
        $('.public-butModal-startTime').val('');
        $('.public-butModal-search input[name=keyword]').val('');
        this.startDate = null;
        this.endDate = null;
    };
