'use strict';
flyer.define('product_type', function (exports, module) {
    var baseDatas = {
        // 用户所在组
    ...
    function initEvent() {
        // 创建商品品类（支持批量）
        $('.createProType').on('click', createProTypeHandle);
        // 删除商品品类（支持批量）
        $('.deleteProType').on('click', deleteProTypeBatchHandle);
    }
     ...
     * @param {any} events 
     */
    function createProTypeHandle(events) {
        flyer.open({
            pageUrl: core.url + '/html/add_base_data.html',
            isModal: true,
            area: [440, 160],
            title: flyer.i18n.initTitle('创建商品品类'),
            btns: [{
                text: flyer.i18n.initTitle('保存'),
                click: function () {
                    var that = this, $firstBtn = $(that.$btns[0]),
                        productType = flyer.exports.baseData.unniqueArr(flyer.exports.baseData.toArray($('#baseDataName').val().trim()));
                    if (productType) {
                        productType = hasProductType(productType, JSON.parse(flyer.store.get('productType')).map(function (item) {
     ...
     * @param {any} events 
     */
    function deleteProTypeBatchHandle(events) {
            ...
            flyer.confirm("确定删除吗?", function (result) {
                    ...
                    btns: [{
                        ...
                        click: function (elm) {
                            ...
                            $.ajax({
                                url: core.url + '/delete_product_type',
                                type: 'POST',
                                data: {
                                    IDS: IDS
                                },
     ...
     * @param {any} events 
     */
    function deleteProTypeSingleHandle(events) {
     ...
     * @param {any} events 
     */
    function updateProTypeHandle(events) {
        var oldProductType = $(events.target).data('producttype'),
            open = flyer.open({
                ...
                isModal: true,
                area: [440, 160],
                title: flyer.i18n.initTitle("更新商品品类"),
                btns: [{
                    text: flyer.i18n.initTitle("保存"),
                    click: function () {
                        var that = this, $firstBtn = $(that.$btns[0]),
                            ID = Number($(events.target).data('id')),
                            productType = $('#baseDataName').val().trim();
                        if (productType) {
                            productType = hasProductType([productType], JSON.parse(flyer.store.get('productType')).map(function (item) {
                                return item.productType;
                            }));
                            if (productType.length) {
                                $.ajax({
                                    url: core.url + '/update_product_type',
                                    type: 'POST',
                                    data: {
                                        ID: ID,
                                    ...
                                    success: function (data, textStatus, jqXHR) {
                                        flyer.msg(data.success ? flyer.i18n.initTitle('操作成功') : flyer.i18n.initTitle('操作失败'));
                                        that.close();
                                        getTableDatas(baseDatas.pagerObj.options.curIndex, Number(baseDatas.pageSizeSelectObj.getSelectedValue()));
        ...
        if ($table && $table.length && Array.isArray(datas)) {
            baseDatas.$table = flyer.table($table, {
                columns: [{
                    ...
                    formatter: function (row) {
                        return '<span class="table-btn" title="' + flyer.i18n.initTitle("编辑") + '"><i class="icon-pen table-btn flyer-edittype-btn" data-id="' + row.ID + '" data-producttype = "' + row.productType + '" style="padding-right:8px;"></i></span>'
                            + '<span class="table-btn" title="' + flyer.i18n.initTitle("删除") + '" style="display:none;"><i class="icon-remove table-btn flyer-deletetype-btn" data-id="' + row.ID + '" style="padding-right:8px;"></i></span>'
                    }
                }],
                data: datas
            });
    ...
    function randerDOMPager($table, datas, total, pagerObj) {
        // 没有数据的时候
        core.tableNoMatch($table, flyer.i18n.initTitle('暂时没有相关商品品类记录'));
        // 初始化下拉框，显示每页数据条数的下拉框
    ...
    function getTableDatas(pageNumber, pageSize) {
        var conditions = {
            ...
            $table = $('#proTypeTable');
        $.ajax({
            url: core.url + '/read_product_type_page',
            type: 'GET',
            data: conditions,
            beforeSend: function (jqXHR, settings) {
    ...
    function getAllProType() {
        $.ajax({
            url: core.url + '/read_product_type',
            type: 'GET',
            data: {
                nocache: window.Date.now(),
                companyOrgID: baseDatas.companyOrgID
            ...
            success: function (data, jqXHR, textStatus) {
                flyer.store.set('productType', JSON.stringify(data.success ? data.data.rows : []));
            },
            error: function (jqXHR, textStatus, errorThrown) {
                flyer.msg(baseDatas.netErrMsg);
            }
