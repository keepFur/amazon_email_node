        ...
        if ($table && $table.length && Array.isArray(datas)) {
            baseDatas.$table = flyer.table($table, {
                columns: [{
                    ...
                    formatter: function(row) {
                        var taskPlant = row.taskPlant === 'TB' ? '淘宝' : '京东';
                        return taskPlant + '（' + core.getTaskParentype(row.taskParentType) + '-' + core.getTaskChildType(row.taskChildType) + '）';
                    }
                }, {
                    title: '任务名称',
                    field: "taskName"
                    ...
                    field: "",
                    formatter: function(row) {
                        return flyer.formatDate('yyyy-mm-dd', row.taskStartDate) + '至' + flyer.formatDate('yyyy-mm-dd', row.taskEndDate);
                    }
                }, {
                    title: '宝贝链接或淘口令',
