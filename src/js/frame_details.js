...
flyer.define("frame_detail", function (exports, module) {
    ...
    function loadEmailInfo(emailInfo) {
        if (emailInfo) {
            ...
            $('.flyer-frame-content').html(emailInfo.html);
    ...
    function loadAttachments(attachmentDatas) {
        ...
        if (Array.isArray(attachmentDatas) && attachmentDatas.length) {
            ...
            $.each(attachmentDatas, function (index, attach) {
                var _hasPreview = /jpg|jpeg|png|pdf|txt|html|docx|doc|xls|xlsx|ppt|pptx/gi.test(attach.md5Name.split('.')[1].toLowerCase()) ? '<i class="icon-search" style="display: inline-block;position:absolute;right:28px;top:8px;" title="' + flyer.i18n.initTitle("附件预览") + '" data-md5name="' + attach.md5Name + '" onclick="window.previewFile(this)"></i>' : '';
