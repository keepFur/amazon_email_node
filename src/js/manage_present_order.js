...
layui.use(['element', 'table', 'layer', 'util', 'form', 'laydate'], function() {
                                    ...
                                    [{
                                        ...
                                        },
                                        {
                                            ...
                                            templet: function(d) {
                                                    ...
                                                    return `<span class="layui-text-${d.status == 1 ? 'green' : 'pink'}">${statusText[d.status]}</span> ${d.status === 1 ? `<a class="layui-btn layui-btn-normal layui-btn-xs js-update-order" data-id="${d.id}"  data-kb-to="${d.addressTo}">修改</a>` : ``}`;
                    }
                }
            ]],
            limits: [10, 20, 50, 100],
    ...
    function getPresentFromStockServer(id) {
        $.get('/api/readFromStock?status=1', function (res) {
            renderPresentStock(res.data.rows,id);
        }, 'json');
    }
    ...
    function renderPresentStock(presentStock,selectedId) {
        ...
        $.each(presentStock, function (index, item) {
            $container.append(`<option value="${item.id}" data-address="${item.address}" ${selectedId==item.id?'selected':''}>${item.name} ${item.address}</option>`);
        });
        form.render('select');
    }
