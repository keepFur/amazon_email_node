...
(function(global, $, factory) {
    ...
    var fly = function() {
        ...
        fn = fly.fn = fly.prototype = {
            ...
            getQueryString: function(name) {
                ...
                // return null;
                var reg = new RegExp(name + "=([^&|#?]+)");
                var r = loca.href.substring(loca.href.indexOf("?") + 1).match(reg);
                if (r !== null) {
    ...
    fn.define = function(namespace, options) {
        ...
        if (fn.isFunction(options)) {
            fn.exports[namespace] = fn.exports[namespace] || {};
            options = options.call(this, fn.exports[namespace], fn.exports);
        }
...
flyer.extend("combobox", function(selector, options) {
     ...
     */
    //定义一个样式数组集合
    var styles = ["flyer-combobox", "flyer-combobox-contents", "filter-options", "fa-angle-down", "flyer-combobox-items", "flyer-combobox-search", "fa-search", "selected", "open", "fa-angle-down", "fa-angle-up", "flyer-disabled"],
        body = document.body,
        doc = document,
        loca = location,
        timer = null,
    ...
    ComboBox.DEFAULTS = {
        ...
        fieldValue: "text",
        //类型: String ,要绑定的值的拼音类型,默认值为 valuePY
        fieldValuePY: "valuePY",

        //类型: String ,要绑定的值拼音，默认为 fieldValuePY
        fieldValuePY: "",
    ...
    ComboBox.prototype = {
        ...
        template: function() {
            ...
            var arryHtmls = [
                ...
                '<input type="text" class="filter-options ' + (opts.disabled === true ? styles[11] : "") + '" name="' + opts.name + '" required="' + opts.required + '" placeholder=' +
                opts.placeholder +
                ...
                '<ul>',
                _this.allowAllItem() ? ("<li data-index='-1' data-key='" + opts.allValue + "' data-value='" + window.escape(opts.allText) + "'><div>" + opts.allText + "</div></li>") : "",
                _this.readerItems(),
                '</ul>',
                '</div>',
                '</div>'
            ...
            this.$search = this[0].find("." + styles[5]);
        ...
        readerItems: function() {
            ...
            for (var i = 0, len = data.length; i < len; i++) {
                arryHtmls.push('<li data-index=' + i + ' data-key=' + data[i][opts.fieldKey] + ' data-value=' + window.escape(data[i][opts.fieldValue]) + '><div>' + data[i][opts.fieldValue] + '</div></li>');
            }
            ...
            _this.$items.each(function() {
                $(this).off("click").on("click", function(e) {
                    ...
                    var item = {
                        ...
                        fieldValue: window.unescape($this.data("value"))
                    }
        ...
        filterData: function(matchValue) {
            ...
            _this.$itemContainer.find("ul").remove();
            _this.$itemContainer.append("<ul>" + _this.readerItems() + "</ul>");
            _this.$items = this.$itemContainer.find("li");
            _this.initEvents();
        },
        ...
        setValue: function(data) {
            ...
            this._data = {
                ...
                fieldValue: data.fieldValue || window.unescape($selectedItem.data("value"))
            }
        ...
        setValues: function(data) {
            ...
            for (var i = 0, len = data.length; i < len; i++) {
                ...
                values = values + (data[i].fieldValue || window.unescape($selectedItem.data("value"))) + opts.multipleSeparator;
                $selectedItem.addClass(styles[7]);
            }
    ...
    Edit.prototype = {
        ...
        template: function() {
            ...
            var aryHtmls = [
                ...
                '<div class="flyer-edit-tool">',
                this.initTools(),
                '</div>',
                '<div class="flyer-edit-content">',
                '<iframe class="flyer-edit-iframe"></iframe>',
                '</div>',
                '</div>'
            ]
            this.parent.append(aryHtmls.join(""));
            this[0].hide();
        },
            ...
            this.elms.tools.each(function(i) {
                $(this).on("click", function() {
                    var $this = $(this),
                        cmd = $this.data("cmd"),
                        name = $this.data("name");
                    $("i[name='" + name + "']").not(this).removeClass("active");
            ...
            this.elms.editer.off("click").on("click", function(e) {
                ...
                if (!$target.hasClass(className)) {
                    ...
                    for (var i = 0, len = tagNames.length; i < len; i++) {
                        cmds.push(tagNames[i].tagName);
                    }
                    cmds = _this.getCommands(cmds);
                    if (cmds.length === 0) {
                        _this.findElements(target, strs);
                        cmds = _this.getCommands(strs);
                    }
                    if (cmds.length >= 0) {
                        _this.elms.container.find("i.active").removeClass("active");
                    }
                    _this.lotCommand(cmds);
                } else {
                    // _this.findElements(target, strs);
                    // if (strs.length > 0) {
                    //     cmds = _this.getCommands(strs);
        ...
        onresize: function() {
            ...
            $('#resize').click(function(e) {
                ...
                if ($(this).hasClass('fa-expand')) {
                    ...
                    _this.elms.content.css('height', _opts.maxHeight);
                    if (typeof _opts.resizeCallback === 'function') {
                        _opts.resizeCallback(true);
                    }
                } else {
                    // 收缩
                    $(this).addClass('fa-expand').removeClass('fa-compress').attr('title', '点击放大');
                    _this.elms.content.css('height', _opts.minHeight);
                    if (typeof _opts.resizeCallback === 'function') {
                        _opts.resizeCallback(false);
                    }
                }
            });
        },
        ...
        findElements: function(elm, result) {
            ...
            if (elm.children && elm.children.length > 0) {
                if (elm.lastChild.tagName) {
                    result.push(elm.lastChild.tagName);
                    this.findElements(elm.lastChild, result);
                }
            }
        },
        ...
        findDictionary: function(str) {
            switch (str) {
                ...
                case "SUB":
                    return "sub";
                default:
                    console.warn(str + " is not find in the dictionary.");
            }
        },
        ...
        initElms: function() {
            ...
            this.elms.toolContainer = this.elms.container.find(".flyer-edit-tool");
            this.elms.tools = this.elms.toolContainer.find("i.fa");
        ...
        initTools: function() {
            var _this = this,
                opts = _this.options,
                aryHtmls = [];
            for (var i = 0, tools = opts.tools, len = tools.length; i < len; i++) {
                aryHtmls.push("<span>");
                for (var j = 0, groups = tools[i].groups, jLen = groups.length; j < jLen; j++) {
                    if (dictionary[groups[j]]) {
                        aryHtmls.push(dictionary[groups[j]].template);
                    }
                }
                aryHtmls.push("</span>");
            }
            if (opts.isResize) {
                aryHtmls.push('<i class="fa fa-expand" title="点击展开" id="resize" style="float:right;"></i>');
            }
            return aryHtmls.join("");
        },
    ...
    Table.prototype = {
        ...
        initHeader: function() {
            ...
            for (var i = 0, columns = this.options.columns, len = columns.length; i < len; i++) {
                ...
                if (column.visible) {
                    if (column.checkbox) {
                        arryHtml.push("<th data-index=" + column._index + "><label class='flyer-table-checkbox'><input name='" + column.field + "' type='checkbox'/><i></i></label></th>");
                    } else if (column.radio) {
                        arryHtml.push("<th data-index=" + column._index + "><label class='flyer-table-radio'><input name='" + column.field + "' type='radio'/><i></i></label></th>");
                    } else {
        ...
        initBody: function() {
            ...
            for (var j = 0, data = this.options.data, jLen = data.length; j < jLen; j++) {
                var arryTd = [];
                arryHtml.push("<tr data-index='" + j + "'>");
                for (var i = 0, columns = this.options.columns, len = columns.length; i < len; i++) {
                    var column = columns[i];
                    if (column.visible) {
                        if (column.checkbox) {
                            arryHtml.push("<td data-index=" + column._index + ">" + (typeof column.formatter === "function" ? column.formatter(data[j], data[j][column.field]) : ("<label class='flyer-table-checkbox'><input name='" + column.field + "' type='checkbox'/><i></i></label>")) + "</td>");
                        } else if (column.radio) {
                            arryHtml.push("<td data-index=" + column._index + ">" + (typeof column.formatter === "function" ? column.formatter(data[j], data[j][column.field]) : ("<label class='flyer-table-radio'><input name='" + column.field + "' type='radio'/><i></i></label>")) + "</td>");
                        } else {
                    ...
                    timer = setInterval(function() {
                        ...
                        if (res) {
                            ...
                            try {
                                ...
                                res = {};
                                return flyer.msg('请对上传接口返回JSON字符:' + e);
                            }
